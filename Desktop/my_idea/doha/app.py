...
def db_connector():
    SERVER, DATABASE, USERNAME, PASSWORD = os.getenv("DB_SERVER"), os.getenv("DB_DATABASE"), os.getenv("DB_USERNAME"), os.getenv("DB_PASSWORD")
...
llm = AzureChatOpenAI(
    ...
    openai_api_version=os.getenv("AZURE_API_VERSION"), azure_deployment=os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME"),
    temperature=0,
)
...
try:
    ...
    with engine.connect() as connection: print("✅ Database connection successful!")
except Exception as e:
    print(f"❌ FAILED TO CONNECT TO DATABASE. Error: {e}"); sys.exit(1)
...
def handle_query():
    try:
        ...
        if not selected_dbs: return jsonify({"error": "No databases were selected."}), 400
        ...
        if not allowed_tables: return jsonify({"summary": "The selected modules have no associated data tables.", "module_counts": {}})
        ...
        for table_name in allowed_tables:
            ...
            if count > 0:
                ...
                detailed_prompt = f"A user is asking about '{user_query}'. Focus on the '{module_name}'. 1. Decide the most relevant columns in the {table_name} table. 2. Write and execute a SQL query for those columns for all matching records. 3. Analyze the data and write a brief explanation. 4. Return a JSON object with 'summary' and 'data' keys."
                ...
                final_details_response = {"aiSummary": ai_data.get("summary"), "tables": [{"title": f"Data for {module_name}", "data": ai_data.get("data")}]}
                api_details_cache[cache_key] = final_details_response
                print(f"✅ Cached details for {cache_key}")
        
        summary_prompt = f"A user searched for '{user_query}'. We found these results: {module_counts}. Write a brief, one-sentence summary of the findings."
        summary_response = llm.invoke(summary_prompt)
        ai_summary = summary_response.content

        return jsonify({ "summary": ai_summary, "module_counts": module_counts })
    except Exception as e:
...
def handle_details():
    data = request.get_json()
    user_query, module = data.get("query"), data.get("module")
    ...
    if cache_key in api_details_cache:
        ...
        return jsonify({"error": "Data not found in cache. This can happen if the initial search was too broad or failed for this module. Please start a new search."}), 404
